// ==UserScript==
// @name       Extrator de dependencias homebrew
// @version    1.1
                ...
                if(alunoatual == aluno){
                    ...
                    do{
                        if(document.getElementById('span_vGRIDGEDALUCOD_00'+zero+linha.toString()) !== null){
                            ...
                            serieHistorico = document.getElementById('span_vGRIDGEDSERIEDSCCPL_00'+zero+linha.toString()).innerHTML;//Texto contendo "...ENSINO MÉDIO > REGULAR > ... > 1º ANO > 10º" etc
                            serieHistorico = extrairAno(serieHistorico); // Extrai apenas a série "1ºANO,2ºANO,etc".
                            //if(situacao == "Em Progressão" || situacao == "Progressão Parcial" || histdep == 1){
                            //if(true==true){ // só para testar
                            if(  situacao == "Em Progressão" || situacao == "Progressão Parcial" || histdep == 1   ||   chaveSerie.includes(aluno+serieHistorico) ){

                                //if (situacao =="Aprovado"){
                                chaveSerie.push(aluno+serieHistorico);

                                //console.log(serieHistorico);
...
function coletar(){
    ...
    vetAluno = txtareaAluno.value.match(/[0-9]+/g).filter(Boolean);
        txtareaDados.value ="Carregando dados...";
...
var output = [['codAluno','nome','ano','série','disciplina','situação']]; var chaves = [];
var outputTemp = {};
    function arrayToHtmlTable(dataArray) {
      ...
      var tabelaHTML = '<head><title>Situação de dependências</title></head><body><table border="1"><thead><tr>';
      ...
      if (dataArray.length > 1) {
        ...
        novaJanela.document.write('<p>Nenhuma informação de DP encontrada pelo script</p>');
      }
    }
        ...
        if(cadastro[lin].length >= 1){
            do{
                ...
                if(parent.frames[0].document.getElementById(area+zero+linhaarea.toString()) !== null){
                    var situ = ''; var discip = ''; var codAluno = ''; var nomeAluno = ''; var serieV = ''; var anoAno = '';

                    codAluno = cadastro[lin][0];
                    nomeAluno = parent.frames[0].document.getElementById('span_vGEDALUNOM').innerHTML.replace(" - ", "");
                   ...
                   // a = a + ";" +seriehist;
                    ...
                    do{
                        if(parent.frames[0].document.getElementById(disciplina+zero+linhadisc.toString()+'00'+zero+linhaarea.toString()) !== null){
                            ...
                            situ = parent.frames[0].document.getElementById(sitdisciplina+zero+linhadisc.toString()+'00'+zero+linhaarea.toString()).innerHTML;
                            discip = parent.frames[0].document.getElementById(disciplina+zero+linhadisc.toString()+'00'+zero+linhaarea.toString()).innerHTML;
                            anoAno = parent.frames[0].document.getElementById("vGEDHISTANO").value
                            if(situ == "Dependente" || chaves.includes(codAluno+discip+seriehist)){
                                //console.log(codAluno,nomeAluno,seriehist,discip,situ);
                                chaves.push(codAluno+discip+seriehist);
                                output.push([codAluno,nomeAluno,anoAno,seriehist,discip,situ]);
                                outputTemp[codAluno+discip+seriehist] = [codAluno,nomeAluno,anoAno,seriehist,discip,situ];
                                if(situ == "Aprovado"){
                                    delete outputTemp[codAluno+discip+seriehist];
                                }
                    ...
                    txtareaDados.value ="Extraindo informações: \n  Estudante "+(lin+1)+" de "+(cadastro.length)+" \n  Etapa "+col+" de "+(cadastro[lin].length - 1);
                    col++;
                    if(col < cadastro[lin].length){
                        console.log(col,cadastro[lin].length);

                        ifrIframe1.src = link+cadastro[lin][col]+","+cadastro[lin][0]+",HWMGedHistorico,,UPD,N";
                    }
                    else if(lin < cadastro.length-1){
                        console.log(lin,cadastro.length);
                        ...
                        //alert('finalizado!');
                        txtareaDados.value = "finalizado!";
                        console.log(outputTemp,output,chaves);
                        console.log(chaveSerie);
                        output = Object.values(outputTemp);
                        output.unshift(['codAluno','nome','ano','série','disciplina','situação']);
...
span1.innerHTML = '(original: Roberson Arruda)';
divCredito.appendChild(span1);
